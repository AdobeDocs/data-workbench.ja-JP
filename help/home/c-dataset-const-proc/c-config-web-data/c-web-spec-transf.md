...
* **[!DNL URI]および [!DNL Page]:** これらのディメンションは、に直接影響を受けま [!DNL Page View Condition]す。指定されたページが[!DNL Page View Condition,]を渡さない場合、URIディメンションやページディメンションには含まれません。

* **[!DNL Visitor Page Views][!DNL Session Page Views]と：** Visitor Page ViewsディメンションとSession Page Viewsディメンションは、それぞれ、特定のセッションに対して訪問者が閲覧したページ数と、特定のセッションで閲覧されたページ数です。[!DNL Page View Condition]で除外されたページは、このカウントには含まれません。

* **Session Number:** は、Session Number [!DNL Page View Condition] ディメンションに間接的に影響します。セッション番号ディメンションは、[!DNL Page View Condition]の前に作成されます。したがって、[!DNL Session Number]と[!DNL Page Views]を比較すると、ページビューのないセッションが存在する可能性があります。

[!DNL Site]のデフォルトの実装には、[!DNL Transformation Dataset Include]ファイルが含まれ、このファイルでページビューの可算ディメンションと関連する[!DNL Page View Condition]が定義されます。
...
1. 必要に応じて、[!DNL Page View Condition]のパラメーターの値を確認または編集します。 以下の例を参考にしてください。このファイルでは、[!DNL Page View Condition]は[!DNL Copy]変換によって定義されます。 このファイルには、Page View 可算ディメンションも定義されていることに注目してください。
...
[!DNL Site] を利用する場合、閲覧された Web サイトページの URI ステムをエレメントに持つ URI ディメンションを定義する必要があります。デフォルトの実装には、URIシンプルディメンションが定義された[!DNL Transformation Dataset Include]ファイルが含まれています。
...
* **Maximum Elements：** URI ディメンションのエレメント（つまり URI）の最大数。デフォルト値は 32768 です。
   ...
   >[!NOTE]
   >
   >この値を変更すると、パフォーマンスに重大な問題が生じる可能性があります。 変更が必要な場合は必ずアドビまでご相談ください。

* ウィンドウ上部の&#x200B;**[!UICONTROL (modified)]**&#x200B;を右クリックし、「**[!UICONTROL Save]**」をクリックして、[!DNL URI.cfg]ファイルを保存します。

* ローカルで作成した変更を反映するには、[!DNL Profile Manager]で[!DNL User]列のファイルのチェックマークを右クリックし、 **[!UICONTROL Save to]** / *&lt;**[!UICONTROL profile name]***&#x200B;をクリックします。profile nameは、データセットインクルードファイルが属するデータセットプロファイルまたは継承プロファイルの名前です。
...
[!DNL Site] を利用する場合、Referrer ディメンションを定義する必要があります。Referrer ディメンションの構成エレメントは、すべてのセッションの最初に出現するログエントリのリファラーの第 2 レベルドメインです。デフォルトの実装には、Referrerシンプルディメンションが定義された[!DNL Transformation Dataset Include]ファイルが含まれています。
...
1. ウィンドウ上部の&#x200B;**[!UICONTROL (modified)]**&#x200B;を右クリックし、「**[!UICONTROL Save]**」をクリックして、[!DNL Referrer.cfg]ファイルを保存します。
...
次のパラメーターは、[!DNL Transformation Dataset Include]ファイルの[!DNL Parameters]ベクトルのメンバーにすることも、[!DNL Transformation.cfg]ファイルに個々のパラメーターとしてリストすることもできる点で一意です。 パラメーターは 1 回しか定義できないので、これらのパラメーターは [!DNL Transformation.cfg] ファイルに定義するか、データセットインクルードファイルの [!DNL Parameters] ベクトルに定義するか、のどちらかとなります。両方のファイルに定義することはできません。**Maximum Session Duration と Session Timeout**
...
[!DNL Site]を使用する場合、通常、デフォルトの実装には[!DNL Transformation Dataset Include]ファイルが含まれており、このファイルに、これらのパラメーターの名前と推奨値が指定されています。

1. データセットプロファイル内で[!DNL Profile Manager]を開き、[!DNL Dataset\Transformation\Traffic\Session Parameters.cfg]に移動します。

   >[!NOTE]
   >
   >[!DNL Site]の実装をカスタマイズした場合、これらのパラメーターが定義されているファイルが、説明されている場所と異なる場合があります。
...
1. ローカルでおこなった変更を有効にするには、[!DNL Profile Manager]で[!DNL User]列のファイルのチェックマークを右クリックし、 **[!UICONTROL Save to]** / **[!UICONTROL profile name]**&#x200B;をクリックします。profile nameは、データセットインクルードファイルが属するデータセットプロファイルまたは継承プロファイルの名前です。
...
[!DNL Site]を使用する場合、デフォルトの実装には、Internal Domainsパラメーターを定義する[!DNL Transformation Dataset Include]ファイルが含まれます。 対象となるパラメーターは、このファイルの中で指定します。必要な作業は、追加したい内部ドメインを入力してファイルを保存するだけです。

1. データセットプロファイル内で[!DNL Profile Manager]を開き、[!DNL Dataset\Transformation\Traffic\Internal Domains.cfg.]に移動します。

   >[!NOTE]
   >
   >[!DNL Site]の実装をカスタマイズした場合、Internal Domainsパラメーターが定義されているファイルが、説明されている場所と異なる場合があります。
