...
1. 内 [!DNL Profile Manager]、 *プロファイル名* 列で、 [!DNL order.txt] ファイルを開き、 **[!UICONTROL Make Local]**.
1. のチェックマークを右クリックします。 [!DNL order.txt] ファイルを [!DNL User] 列とクリック **[!UICONTROL Open]** > **[!UICONTROL in Notepad]**. この [!DNL order.txt] ファイルが表示されます。

   ![ステップ情報](assets/cfg_ordertxt.png)

1. （オプション） [包含] または [排他] を設定します。 この設定は、 [!DNL order.txt] ファイル内に存在するが、 [!DNL Profile Manager] がメニューに表示されます。 次のオプションがあります。

   * **[包含]:** これはデフォルト設定です。 この設定では、[!DNL order.txt] ファイルに指定されていないメニュー項目が、メニューの下部にアルファベット順に表示されます。例えば、 [!DNL Profile Manager] には、 [!DNL order.txt] 上の例では、プロファイルは「Data」の下に表示されます。

   * **[排他]:** この設定により、 [!DNL order.txt] ファイルがメニューから除外されました。 例えば、 [!DNL Profile Manager] には、 [!DNL order.txt] 上記の場合、プロファイルはメニューのどこにも表示されません。

   * **空白：** どちらも [包含] または [排他] はファイルの上部に表示され、Data Workbenchは、設定が [包含].
...
1. 次の手順を実行します。
   ...
   <table id="table_C5D5313DF5E4470499B0B285BA2690F0"> 
    ...
    <td colname="col2"> <p>アイテム名をData Workbenchに表示する順に入力します。 </p> <p>例えば、各メニュー項目名が対応するファイルまたはフォルダーの名前と一致すれば、次のように指定すると「<b>テーブル</b>」が最初に、「<b>ビジュアライゼーション</b>」、「<b>凡例</b>」、「<b>注意</b>」が最後に表示されます。 </p> <p><b>テーブル</b> </p> <p><b>ビジュアライゼーション </b> </p> <p><b>凡例 </b> </p> <p><b>注意</b> </p> </td> 
    </tr> 
    <tr> 
...
1. ファイルを保存して閉じます。
1. （オプション）作業プロファイルのすべてのユーザーが変更を利用できるようにするには、 [!DNL order.txt] ファイルを [!DNL User] 列とクリック **[!UICONTROL Save to]** > * **[!UICONTROL working profile name]**.
