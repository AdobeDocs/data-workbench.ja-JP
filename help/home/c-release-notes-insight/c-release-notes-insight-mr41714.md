---
description: Adobe Analyticsは、Data Workbenchを強化するデータフィードを更新および拡張し、Adobe Experience Cloud内のプロファイルとオーディエンスを活用します。 2014 年 4 月 21 日までに、Data Workbench のすべてのユーザーはこの新しいデータフィード形式に移行するための準備を行う必要があります。
title: April MR 2014 の更新
...
# April MR 2014 の更新{#updates-for-april-mr}

Adobe Analyticsは、Data Workbenchを強化するデータフィードを更新および拡張し、Adobe Experience Cloud内のプロファイルとオーディエンスを活用します。 2014 年 4 月 21 日までに、Data Workbench のすべてのユーザーはこの新しいデータフィード形式に移行するための準備を行う必要があります。

プロファイルとオーディエンスは、Adobe Experience Cloudおよびサードパーティのサービス全体で使用できる、顧客の完全で実用的なビューを提供します。 Analytics内で新機能の基盤の確立を開始するために、今後の4月のMRリリースで新しいExperience Cloud IDサービスが、その他の機能強化や改善と共に、データフィードに追加されます。 この新しいサービスは、Adobe Experience Cloudを使用するすべてのお客様が利用できます。

この新しいデータフィードで提供される追加の機能強化とバグの修正については、Data Workbench の設定を準備するための手順と共に、このトピックに記載されています。

## 新しいデータフィードの機能強化とバグの修正 {#section-0013de07da93411d929a1dfd0417f61b}

* Experience Cloud IDサービスの追加を参照してください。
* Analytics 内でターゲットレポートと分析をサポートするために、「ターゲットアクション」フィールドを追加しました。
* 製品の行タイプに [!DNL bot_id] 値を組み込むことで、Report &amp; Analytics との整合性を向上させました。
* データ値の整合性を失うことなくマーチャンダイジング eVar のサイズを縮小しました。
* ハッシュ化された値がマーチャンダイジング eVar に表示されることがある問題を解決しました。
* 引き続き安定性とパフォーマンスを向上させるために、プロセス内で内部的な機能強化を行いました。

## Data Workbench の設定の更新 {#section-381fcd185af6474bb656501c096c451d}

Data Workbench の変更は、移行後にデータフィードの統合が中断されないようにするためのものです。次のスケジュールされた再処理時に、設定のログ処理段階の現在のデコーダーグループを更新します。Data Workbenchアーキテクチャの管理を担当するチームは、新しいExperience Cloud訪問者IDを公開し、利用するのに役立つ追加のタスクを実行できます。

**Data Workbench の更新タスク**

1. 現在のデコーダーグループを探します。通常、プロファイルマネージャー内の Dataset\Log Processing\Decoding [!DNL Instructions.cfg] にあります。
1. 設定ファイル内で、現在のデコーダー形式を右クリックし、「コピー」を選択します。
1. 右クリックし、現在のデコーダー形式の下に新しいエントリを貼り付けます。
1. 新しいデコーダー形式を展開し、リストの最後に 2 つの新しいフィールド位置を追加します。データセットの新しいフィールドを使用する準備ができていない場合は、これらの位置を空のままにすることができます。それ以外の場合は、次のように定義します。

   * [!DNL x-mcvisid]
   * [!DNL x-tnt-action]

## デコード命令 {#section-ba63132ce9604851ad319b6181011820}
