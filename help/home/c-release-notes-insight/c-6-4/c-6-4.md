...
| [ワークステーションセットアップウィザード](/help/home/c-install-insight/install-setup/dwb-client-installer.md) | ステップバイステップ方式のインストールウィザードを使用して Data Workbench（クライアント）をセットアップします。ワークベンチのダウンロード、オプション設定、インストール、サーバーとの同期を行うことができます。 |
| ワークステーション設定エクスペリエンスの強化 | ワークステーションのインストール後、「サーバーへの接続を設定」ワークスペースが開き、Insight.cfg ファイル内のサーバー接続情報の入力に関する詳しい情報が表示されます。サーバーへの接続状況の表示やプロファイルの選択も行えます。 |
| [プレゼンテーションレイヤー](/help/home/c-get-started/c-vis/c-present-layer.md) | プレゼンテーションオーバーレイを使用して、ビジュアライゼーションへの注釈の付与やビジュアライゼーションの明確化を行うことができます。テキストコールアウト、矢印、画像および色分けの追加によってデータを強調および明確化したうえで、他のユーザーと共有できます。 |
...
| [新しいユーザーインターフェイス機能 ](/help/home/c-adobe-data-workbench-dashboard/t-getting-started/c-dashboard-user-interface.md) | Data Workbench 6.4では、新しいワークスペースアイコン、ツールヒント、スプラッシュスクリーンおよびF1ショートカットがヘルプに追加されました。 また、ツールバーからヘルプ／トレースディレクトリを開くを選択してログファイルを開くこともできます。 |
| クラスタリングアルゴリズムの更新 | クラスタリング機能に期待値最大化が追加されました。これは Adobe Analytics Premium の機能です。 |
| ログ情報の更新 | Data Workbench では、拡張されたログフレームワーク「L4」を使用できます。このフレームワークでは、必要に応じてログの設定を行うことができます。6.4 パッケージに付随するデフォルトの実装は、このソフトウェアの処理に関する重要な情報を提供しています。サーバーイベントのトラブルシューティングや根底にある問題の分析に役立てるために、関連するサーバー、クライアントおよびレポートサーバーなどの詳細情報をログに追加することができます。追加の L4 ログを実装するうえでさらなるサポートが必要な場合は、アカウントマネージャーにお問い合わせください。 |
...
| グラフビジュアライゼーションでのズーム機能 | ズーム機能を使用すると、値どうしの差が大きくなっても指標ラベルが適切に表示されます。これまでは、例えば指標の回帰値をそれまでの値よりも高く設定すると、値のコントラストの変化によってラベルが消えていました。You can now zoom in to the visualization by clicking **Ctrl** and moving the mouse wheel while hovering over the graph. |
| 新しいカラーピッカーツール]() | 新しいカラーピッカーを使用すると、単純なカラーパレットからカラーを選択したり、「詳細」タブをクリックしてグラデーションスケールからの選択または独自の RGB 値の入力を行ったりできます。 |
...
| Insight.exeとInsightSetup.exeが電子署名されるようになりました | これらの実行ファイルは、ソフトウェアダウンロードが変更または破壊されていないことを証明するためにデジタル署名されるようになりました。 |
...
| ファイルビジュアライゼーションの分離 | レポートを表示する際に、基本プロファイルのファイルビジュアライゼーション（管理者／ファイル）に大きなディレクトリ（削除されたログ、エクスポート、検索データ）が含まれなくなります。これにより、レポートの表示が速くなります。大きなディレクトリはそれぞれ個別のレポート（管理者／エクスポートファイル、検索ファイル、ログファイル）に表示されるようになります。 |
...
>[!NOTE]
>
>ツールバーのアイコンをオフにして、テキスト名に戻すことができます。 Add the **[!DNL Toolbar Icons = bool: false]** [parameter to the Insight.cfg file](https://docs.adobe.com/content/help/en/data-workbench/using/client/c-insght-config-param.html). false に設定した場合、ワークステーションのユーザーインターフェイスにアイコンが表示されなくなり、ツールバーにテキストでメニュー名が表示されます。
...
* 以下の実行ファイルについて、Windows 2012 Server の **MS System Center Endpoint Protection** の&#x200B;***除外プロセス***&#x200B;を&#x200B;*追加*&#x200B;します。
   ...
   * **`ExportIntegration.exe`**
   これにより、これらのインターフェイスする実行可能ファイルに対するホワイトリストの権限が許可されます。
...
* **フォントを Report Server に追加します。** **`ReportServer.cfg`**ファイルに、（すべての言語で）次のフォントを追加します。
...
* 同じワークスペースで異なる入力を再実行した場合に傾向スコアが再設定されていませんでした。今回の修正によって再設定が適切に行われるようになりました。
