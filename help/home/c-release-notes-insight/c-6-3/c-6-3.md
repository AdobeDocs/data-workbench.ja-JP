...
* **Meta.cfg ファイルの更新**（[!DNL E:\..\プロファイル\`<your custom profile>&#39;\Context\meta.cfg]) を使用して、ファイルシステムユニット（FSU サーバ）の更新されたパスワード暗号化を設定し、名前と値のペアの変換のエントリを追加し、 [クエリー文字列のグループ化](../../../home/c-get-started/c-admin-intrf/c-query-que/c-query-string-grouping.md).
