...
1. Webサーバーまたはアプリケーションサーバーに管理者アクセス権を持っている場合は、Webクラスター内の任意の [!DNL Sensor][!DNL Sensor] マシン上のインストールフォルダーに移動します。 管理者アクセス権がない場合は、Adobeのアカウントマネージャーに問い合わせて、ファイルを要求してください。
...
| テストの最小時間（日） | 指定した信頼水準、電力レベル、Zスコアを満たすためにテストを実行する必要がある最小日数。 この計算された数値は、「期間ごとの予測訪問者数」フィールドで説明したのと同じ問題に従います。 再訪問者の多いWebサイトの場合、「Minimum Experiment Time (Days)」フィールドは、個別訪問者の数が「Minimum Experted」フィールドの値と等しくなるまでに予想される日数です。 |
| 訪問者 | 訪問者値の見出しです。 |
...
と比較し、 [!DNL Visitors] 列の合計フィールドの値と比較します。
...
1. レコード用にファイルを保存し、そのファイルの情報を使用してテスト設定スプレッドシートを使用してテストを設定します。 このスプレッドシートの詳細については、「テストの [設定と導入](../../home/c-undst-ctrld-exp/t-crt-ctrld-exp/c-cnfg-dply-exp.md#concept-50f1de0242904698937bb72b3ea1b429)」を参照してください。
